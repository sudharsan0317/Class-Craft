...
import { Plus, Trash2, HelpCircle, Check, Sparkles, Loader2 } from 'lucide-react';
...
export default function QuizBuilder({ phaseId, questions = [] }) {
  ...
  // Get phase context for the AI
  const phase = useLessonStore((state) => 
    state.editedLesson?.phases?.find(p => p.id === phaseId)
  );
  ...
  const handleAIGenerate = async () => {
    ...
    try {
      const data = await generateQuizQuestions({
        topic: phase.title,
        context: phase.instructions || "Generate diagnostic multiple choice questions.",
        num_questions: 3
      });
      
      if (data.questions && data.questions.length > 0) {
        // Appending the generated questions to the store
        // addQuestion usually adds a blank one, we might need a custom store action 
        // or we just call updateQuestion if addQuestion returns an ID.
        // Wait, the existing store might only add empty questions. Let's look at it.
        // We can just add them and then update them, or if the store allows passing a full question object:
        data.questions.forEach((q) => {
          addQuestion(phaseId, q);
        });
      }
    } catch (error) {
      alert("Failed to generate quiz questions: " + error.message);
    } finally {
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-white bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 transition-colors shadow-2xs cursor-pointer"
          >
            {isGenerating ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Sparkles className="w-3.5 h-3.5" />}
            <span>{isGenerating ? 'Generating...' : 'Auto-Generate AI Quiz'}</span>
          </button>
