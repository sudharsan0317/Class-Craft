...
export default function LoginPage() {
  ...
  // Sign In states
  const [email, setEmail] = useState('teacher@classcraft.edu');
  ...
  const handleLoginSubmit = (e) => {
    ...
    setTimeout(() => {
      login({ name: 'Teacher Alex', email: email || 'alex@classcraft.edu' });
      setIsSubmitting(false);
    }, 400);
  };

  const handleSocialLogin = (provider) => {
    login({ name: `Teacher (${provider})`, email: `teacher@${provider.toLowerCase()}.com` });
  };
  ...
  const handleCompleteSetup = (e) => {
    e.preventDefault();
    if (setUserNameInStore && signupUsername.trim()) {
      setUserNameInStore(signupUsername.trim());
    }
    login({
      name: signupUsername.trim() || 'Educator',
      email: signupEmail.trim() || 'teacher@classcraft.edu'
    });
  };
